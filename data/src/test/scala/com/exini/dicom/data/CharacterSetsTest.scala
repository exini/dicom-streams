...
class CharacterSetsTest extends AnyFlatSpecLike with Matchers {
  ...
  "Parsing a DICOM file" should "parse an Arab name correctly" in {
    ...
    val nameBytes      = bytes(0xe2, 0xc8, 0xc7, 0xe6, 0xea, 0x5e, 0xe4, 0xe6, 0xd2, 0xc7, 0xd1)
  ...
  it should "parse a French name correctly" in {
    ...
    val nameBytes      = bytes(0x42, 0x75, 0x63, 0x5e, 0x4a, 0xe9, 0x72, 0xf4, 0x6d, 0x65)
  ...
  it should "parse a German name correctly" in {
    ...
    val nameBytes      = bytes(0xc4, 0x6e, 0x65, 0x61, 0x73, 0x5e, 0x52, 0xfc, 0x64, 0x69, 0x67, 0x65, 0x72)
    val charsets       = new CharacterSets(Seq("ISO_IR 100"))
    val name           = charsets.decode(VR.PN, nameBytes)
    checkPersonName(name, nameCodePoints)
  }
  ...
  it should "parse a Greek name correctly" in {
    ...
    val nameBytes      = bytes(0xc4, 0xe9, 0xef, 0xed, 0xf5, 0xf3, 0xe9, 0xef, 0xf2)
  ...
  it should "parse a Japanese name correctly (1)" in {
    ...
    val nameBytes = bytes(0x59, 0x61, 0x6d, 0x61, 0x64, 0x61, 0x5e, 0x54, 0x61, 0x72, 0x6f, 0x75, 0x3d, 0x1b, 0x24,
  ...
  it should "parse a Japanese name correctly (2)" in {
    ...
    val nameBytes = bytes(0xd4, 0xcf, 0xc0, 0xde, 0x5e, 0xc0, 0xdb, 0xb3, 0x3d, 0x1b, 0x24, 0x42, 0x3b, 0x33, 0x45,
  ...
  it should "parse a Hebrew name correctly" in {
    ...
    val nameBytes      = bytes(0xf9, 0xf8, 0xe5, 0xef, 0x5e, 0xe3, 0xe1, 0xe5, 0xf8, 0xe4)
  ...
  it should "parse a Korean name correctly" in {
    ...
    val nameBytes = bytes(0x48, 0x6f, 0x6e, 0x67, 0x5e, 0x47, 0x69, 0x6c, 0x64, 0x6f, 0x6e, 0x67, 0x3d, 0x1b, 0x24,
  ...
  it should "parse a Russian name correctly" in {
    ...
    val nameBytes      = bytes(0xbb, 0xee, 0xda, 0x63, 0x65, 0xdc, 0xd1, 0x79, 0x70, 0xd3)
  ...
  it should "parse a Chinese name correctly (1)" in {
    ...
    val nameBytes = bytes(0x57, 0x61, 0x6e, 0x67, 0x5e, 0x58, 0x69, 0x61, 0x6f, 0x44, 0x6f, 0x6e, 0x67, 0x3d, 0xe7,
  ...
  it should "parse a Chinese name correctly (2)" in {
    ...
    val nameBytes = bytes(0x57, 0x61, 0x6e, 0x67, 0x5e, 0x58, 0x69, 0x61, 0x6f, 0x44, 0x6f, 0x6e, 0x67, 0x3d, 0xcd,
