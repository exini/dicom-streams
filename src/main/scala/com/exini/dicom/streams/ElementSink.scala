...
object ElementSink {
  ...
  /**
    ...
    */
  private case class ElementSinkData(
      builderStack: Seq[ElementsBuilder] = Seq(Elements.newBuilder()),
  ...
  def elementSink(implicit ec: ExecutionContext): Sink[Element, Future[Elements]] =
    Flow[Element]
      .toMat(
        Sink
          .fold[ElementSinkData, Element](ElementSinkData()) {
            case (sinkData, element) =>
              element match {
                ...
                case sequenceElement: SequenceElement =>
                  sinkData.pushSequence(
                    Sequence
                      .empty(sequenceElement)
                ...
                case itemElement: ItemElement if sinkData.hasSequence =>
                  val builder = sinkData.builderStack.head
                  val sequence = sinkData.sequenceStack.head + Item
                    .empty(itemElement)
