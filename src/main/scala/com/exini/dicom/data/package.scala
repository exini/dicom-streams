...
package object data {
  ...
  def bytesToShort(bytes: ByteString, bigEndian: Boolean = false): Short =
  ...
  def bytesToLong(bytes: ByteString, bigEndian: Boolean = false): Long =
    if (bigEndian) bytesToLongBE(bytes) else bytesToLongLE(bytes)
  def bytesToLongBE(bytes: ByteString): Long =
  ...
  def bytesToLongLE(bytes: ByteString): Long =
  ...
  def bytesToDouble(bytes: ByteString, bigEndian: Boolean = false): Double =
  ...
  def bytesToFloat(bytes: ByteString, bigEndian: Boolean = false): Float =
  ...
  def bytesToUShort(bytes: ByteString, bigEndian: Boolean = false): Int =
  ...
  def longToBytes(i: Long, bigEndian: Boolean = false): ByteString =
    if (bigEndian) longToBytesBE(i) else longToBytesLE(i)
  def longToBytesBE(i: Long): ByteString =
    ByteString(
      (i >> 56).toByte,
      (i >> 48).toByte,
      (i >> 40).toByte,
  ...
  def longToBytesLE(i: Long): ByteString =
    ByteString(
      i.toByte,
      (i >> 8).toByte,
      (i >> 16).toByte,
  ...
  def floatToBytes(f: Float, bigEndian: Boolean = false): ByteString =
    intToBytes(java.lang.Float.floatToIntBits(f), bigEndian)
  def doubleToBytes(d: Double, bigEndian: Boolean = false): ByteString =
    ByteString(
      ByteBuffer
  ...
  def tagToBytes(tag: Int, bigEndian: Boolean = false): ByteString =
    if (bigEndian) tagToBytesBE(tag) else tagToBytesLE(tag)
  def tagToBytesBE(tag: Int): ByteString = intToBytesBE(tag)
  def tagToBytesLE(tag: Int): ByteString =
    ByteString((tag >> 16).toByte, (tag >> 24).toByte, tag.toByte, (tag >> 8).toByte)

  def intToUnsignedLong(i: Int): Long   = i & 0xffffffffL
  def shortToUnsignedInt(i: Short): Int = i & 0xffff

  def truncate(n: Int, bytes: ByteString, bigEndian: Boolean = false): ByteString =
    if (bigEndian) bytes.drop(n) else bytes.dropRight(n)

  private def bytesToHexString(bytes: ByteString) =
    new String(bytes.flatMap(b => List(hexDigits(b >>> 4 & 0xf), hexDigits(b & 0xf))).toArray)
  ...
  def item(length: Int, bigEndian: Boolean): ByteString =
  ...
  def itemDelimitation(bigEndian: Boolean = false): ByteString =
  ...
  def sequenceDelimitation(bigEndian: Boolean = false): ByteString =
